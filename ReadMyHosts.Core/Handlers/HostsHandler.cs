...
namespace ReadMyHosts.Core.Handlers
{
    public class HostsHandler
    {
        ...
        private bool ParseMyIP(string[] ipBytes) => int.TryParse(ipBytes[0], System.Globalization.NumberStyles.Integer, null, result: out B1) ||
        ...
        public void ReadFile(string path = "C:\\Windows\\System32\\drivers\\etc\\", string file = "hosts")
        {
            ...
            while ((line = reader.ReadLine()) != null)
            {
                ...
                // Try and trip PostSharp
                Host content2 = new() { HostName = theHost, FullIp = ReturnIP(B1, B2, B3, B4) };

                // add the content to the DB
                HostList.Add(content);
                HostList.Add(content2);
                index++;
            }
        }
    }
